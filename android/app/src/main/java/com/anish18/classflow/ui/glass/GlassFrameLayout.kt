...
import com.anish18.classflow.ui.glass.filters.GlassFilter
import com.anish18.classflow.ui.glass.renderer.GlassRenderer
import com.anish18.classflow.ui.glass.utils.SpringAnimator
 ...
 * ## Quick start
 * ```xml
 * <com.anish18.classflow.ui.glass.GlassFrameLayout
 ...
 * ```
 * ```kotlin
 * GlassLiquidGlass.applyBase(myGlassView)   // apply the canonical iOS recipe
 ...
 * [setOnClickListener] so subclasses such as [GlassSwitch] are not affected.
 *
 * ## Threading
 ...
 *
 * @see GlassLiquidGlass.applyBase
 * @see updateBackground
 *
 * @author Saurav Sajeev
 */
@SuppressLint("ClickableViewAccessibility")
open class GlassFrameLayout @JvmOverloads constructor(
    context: Context,
    ...
    private val renderer: GlassRenderer = GlassRenderer(context)
    private var lastWidth = 0
    private var lastHeight = 0
    private var captureScheduled = false
    private var debug = false
    private var lastCaptureTime = 0L
    private val minCaptureInterval = 120L
    ...
    private val glowCallback: Choreographer.FrameCallback = object : Choreographer.FrameCallback {
        override fun doFrame(frameTimeNanos: Long) {
            val dt = if (glowLastNanos == 0L) 0.016f
            else ((frameTimeNanos - glowLastNanos) / 1_000_000_000f).coerceAtMost(0.048f)
            glowLastNanos = frameTimeNanos
            ...
            if (!settled) Choreographer.getInstance().postFrameCallback(this)
        }
    }
    ...
    init {
        ...
        outlineProvider = object : android.view.ViewOutlineProvider() {
            override fun getOutline(view: View, outline: android.graphics.Outline) {
                outline.setRoundRect(0, 0, view.width, view.height, clipRadius)
            }
        }
        clipToOutline = true

        val density = context.resources.displayMetrics.density
        context.theme.obtainStyledAttributes(attrs, R.styleable.GlassFrameLayout, 0, 0).apply {
            try {
                getFloat(R.styleable.GlassFrameLayout_pfl_glassWidth, -1f).takeIf { it > 0 }
                    ?.let { w ->
                        getFloat(
                            R.styleable.GlassFrameLayout_pfl_glassHeight,
                            -1f
                        ).takeIf { it > 0 }?.let { h ->
                            setGlassSize(w, h)
                        }
                ...
                setIOR(getFloat(R.styleable.GlassFrameLayout_pfl_ior, 1.42222f))
                setThickness(
                    getDimension(
                        R.styleable.GlassFrameLayout_pfl_glassThickness,
                        18f * density
                    )
                )
                setNormalStrength(
                    getFloat(
                        R.styleable.GlassFrameLayout_pfl_normalStrength,
                        3.6515f
                    )
                )
                setDisplacementScale(
                    getFloat(
                        R.styleable.GlassFrameLayout_pfl_displacementScale,
                        1f
                    )
                )
                setHeightBlurFactor(
                    getFloat(
                        R.styleable.GlassFrameLayout_pfl_heightTransitionWidth,
                        15.3f
                    )
                )
                setMinSmoothing(getFloat(R.styleable.GlassFrameLayout_pfl_minSmoothing, 4.0f))
                setBlurRadius(getFloat(R.styleable.GlassFrameLayout_pfl_blurRadius, 2f))
                setHighlightWidth(getFloat(R.styleable.GlassFrameLayout_pfl_highlightWidth, 0.35f))
                setChromaticAberration(
                    getFloat(
                        R.styleable.GlassFrameLayout_pfl_chromaticAberration,
                        26f
                    )
                )
                setBrightness(getFloat(R.styleable.GlassFrameLayout_pfl_brightness, 1.21f))
                setShowNormals(getBoolean(R.styleable.GlassFrameLayout_pfl_showNormals, false))
                setCornerRadius(
                    getDimension(
                        R.styleable.GlassFrameLayout_pfl_cornerRadius,
                        28f * density
                    )
                )
                setShadowProperties(
                    "#00000000".toColorInt(),
                    getFloat(R.styleable.GlassFrameLayout_pfl_shadowSoftness, 10f)
                )
                setGlassColor(getColor(R.styleable.GlassFrameLayout_pfl_glassColor, Color.TRANSPARENT))
                setLightDirection(
                    getFloat(R.styleable.GlassFrameLayout_pfl_lightDirX, 1.0f),
                    getFloat(R.styleable.GlassFrameLayout_pfl_lightDirY, 1.0f)
                )
                setSpecular(
                    getFloat(R.styleable.GlassFrameLayout_pfl_specular, 2.4184f),
                    getFloat(R.styleable.GlassFrameLayout_pfl_shininess, 152.6f)
                )
                setRimStrength(getFloat(R.styleable.GlassFrameLayout_pfl_rimStrength, 0.18f))
                setDispersion(
                    getFloat(R.styleable.GlassFrameLayout_pfl_dispersionR, 0.39000002f),
                    getFloat(R.styleable.GlassFrameLayout_pfl_dispersionB, 0.375f)
                )
                setCausticIntensity(
                    getFloat(
                        R.styleable.GlassFrameLayout_pfl_causticIntensity,
                        0.1445f
                    )
                )
                setTransmittance(getFloat(R.styleable.GlassFrameLayout_pfl_transmittance, 1.0f))
                setLiquidDomeStrength(
                    getFloat(
                        R.styleable.GlassFrameLayout_pfl_liquidDome,
                        1.3f
                    )
                )
                setFresnelReflectStrength(
                    getFloat(
                        R.styleable.GlassFrameLayout_pfl_fresnelReflect,
                        1.98f
                    )
                )
                setLensRefractionScale(
                    getFloat(
                        R.styleable.GlassFrameLayout_pfl_lensRefractionScale,
                        1.2973684f
                    )
                )
                getInt(R.styleable.GlassFrameLayout_pfl_captureDownsample, -1)
                    .takeIf { it >= 0 }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        for (i in 0 until childCount) {
            ...
            if (child != glSurface && child.visibility != GONE) {
                measureChildWithMargins(child, widthMeasureSpec, 0, heightMeasureSpec, 0)
                val lp = child.layoutParams as? ViewGroup.MarginLayoutParams
                val marginH = lp?.let { it.leftMargin + it.rightMargin } ?: 0
                val marginV = lp?.let { it.topMargin + it.bottomMargin } ?: 0
                maxChildHeight = maxOf(maxChildHeight, child.measuredHeight + marginV)
                maxChildWidth = maxOf(maxChildWidth, child.measuredWidth + marginH)
            }
        }
        ...
        val resolvedHeight = resolveSize(maxChildHeight, heightMeasureSpec)

        val glWidthSpec = MeasureSpec.makeMeasureSpec(resolvedWidth, MeasureSpec.EXACTLY)
        val glHeightSpec = MeasureSpec.makeMeasureSpec(resolvedHeight, MeasureSpec.EXACTLY)
        glSurface.measure(glWidthSpec, glHeightSpec)
     ...
     * (e.g. [GlassSwitch]) can keep their own touch handling without conflict.
     */
    ...
    /**
     * Marks that a parent view (e.g. [GlassSlider]) owns backdrop updates.
    ...
    /**
     * When set (e.g. to a [com.anish18.classflow.ui.glass.GlassSwitch]), background capture draws this host
    ...
    internal fun captureAndSetBackground() {
        ...
        if (visibility != VISIBLE || !isAttachedToWindow || !isShown) return
    ...
    private fun captureFromHost(host: ViewGroup) {
        ...
        val needsScaleReset = savedScaleX != 1f || savedScaleY != 1f
        if (needsScaleReset) {
            scaleX = 1f
            scaleY = 1f
        }
        ...
        alpha = 0f
        canvas.drawColor(if (com.anish18.classflow.ui.theme.ThemeState.isDark) Color.BLACK else Color.WHITE)
        host.draw(canvas)
        alpha = savedAlpha
    ...
    fun setGlassColor(color: Int) {
        glSurface.queueAndRender { renderer.setGlassColor(color) }
        post { glSurface.setBackgroundColor(color) }
    }

    fun setFilter(value: GlassFilter) = glSurface.queueAndRender { renderer.setFilter(value) }
