 ...
 *
 * ## Appearance
 * Applies [GlassLiquidGlass.applyBase] then scales thickness, blur band, and refraction inset
...
class GlassIconButton @JvmOverloads constructor(
    ...
    private fun applyPressState(t: Float) {
        glassSurfaceView.setBlurRadius(lerp(restBlur, 0f, t))
        glassSurfaceView.setChromaticAberration(lerp(0f, 3.5f, t))
        glassSurfaceView.setLensRefractionScale(lerp(restLensScale, restLensScale + 0.65f, t))
    }
    ...
    private val touchListener = OnTouchListener { _, event ->
        when (event.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                ...
                glassSurfaceView.showGlow(event.x, event.y)
    ...
    init {
        ...
        context.theme.obtainStyledAttributes(attrs, R.styleable.GlassIconButton, 0, 0).apply {
            try {
                GlassLiquidGlass.applyBase(glassSurfaceView)
                with(glassSurfaceView) {
                    setLiquidDomeStrength(getFloat(R.styleable.GlassIconButton_pib_liquidDomeStrength, 0.92f))
                    setFresnelReflectStrength(getFloat(R.styleable.GlassIconButton_pib_fresnelReflectStrength, 1.8f))
                    setShadowProperties(
                        getColor(R.styleable.GlassIconButton_pib_shadowColor, 0),
                        getFloat(R.styleable.GlassIconButton_pib_shadowSoftness, 0.18f)
                    )
                    setIOR(getFloat(R.styleable.GlassIconButton_pib_ior, 1.55f))
                    setNormalStrength(getFloat(R.styleable.GlassIconButton_pib_normalStrength, 1.6f))
                    setDisplacementScale(getFloat(R.styleable.GlassIconButton_pib_displacementScale, 1.15f))
                    setChromaticAberration(getFloat(R.styleable.GlassIconButton_pib_chromaticAberration, 0f))
                    setBrightness(getFloat(R.styleable.GlassIconButton_pib_brightness, 1.12f))
                    setHighlightWidth(getFloat(R.styleable.GlassIconButton_pib_highlightWidth, 1.2f))
                    setRimStrength(1.4f)
                    setShowNormals(getBoolean(R.styleable.GlassIconButton_pib_showNormals, false))
                }

                restBlur = getFloat(R.styleable.GlassIconButton_pib_blurRadius, 2f)
                restLensScale = getFloat(R.styleable.GlassIconButton_pib_lensRefractionScale, 0.85f)
                glassSurfaceView.setBlurRadius(restBlur)
                glassSurfaceView.setLensRefractionScale(restLensScale)

                defaultSizePx = getDimension(R.styleable.GlassIconButton_pib_buttonSize, dp(56f)).toInt()
                val iconPadding = getDimension(R.styleable.GlassIconButton_pib_iconPadding, dp(8f)).toInt()
                val iconRes = getResourceId(R.styleable.GlassIconButton_pib_iconSrc, 0)
                val iconTint = getColor(R.styleable.GlassIconButton_pib_iconTint, Color.BLACK)
                pressScale = getFloat(R.styleable.GlassIconButton_pib_pressScale, 0.82f)
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        glassSurfaceView.setCornerRadius(side / 2f)
    ...
    private fun applySizeScaledGlass(sidePx: Int) {
        ...
        glassSurfaceView.setThickness((sideDp * 0.20f).coerceIn(6f, 18f) * density)
        glassSurfaceView.setHeightBlurFactor((sideDp * 0.32f).coerceIn(12f, 30f) * density)
        glassSurfaceView.setRefractionInset((sidePx * 0.04f).coerceIn(2f * density, 5f * density))
    }

    /** Refreshes the glass background texture. Call after the content behind this button changes. */
    fun updateBackground() = glassSurfaceView.updateBackground()
    ...
    fun setIOR(value: Float) {
        glassSurfaceView.setIOR(value); glassSurfaceView.updateBackground()
    }
    ...
    val glassSurface: GlassFrameLayout get() = glassSurfaceView

    override fun setOnClickListener(l: OnClickListener?) {
        clickListener = { l?.onClick(this) }
        glassSurfaceView.setGlowEnabled(l != null)
    }
}