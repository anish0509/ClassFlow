package com.anish18.classflow.ui.glass.filters
...
/**
 * Preset filter styles for [GlassFrameLayout], defining calibrated glass-optical parameters.
 *
 ...
 * ### Apply via:
 * ```kotlin
 * filter.applyTo(glassView)
 * // or
 * glassView.applyFilter(GlassFilter.CRYSTAL)
 * ```
 *
 * @author Saurav Sajeev
 */
data class GlassFilter(
    val name: String,
    ...
    /**
     * Applies all properties of this filter to the given [GlassFrameLayout].
     */
    fun applyTo(glass: GlassFrameLayout) {
        glass.setRefractionInset(refractionInset)
        ...
        glass.setHeightBlurFactor(heightBlurFactor)
        glass.setMinSmoothing(minSmoothing)
        glass.setEdgeRefractionFalloff(edgeRefractionFalloff)
        glass.setLiquidDomeStrength(liquidDomeStrength)
        glass.setFresnelReflectStrength(fresnelReflectionStrength)
        glass.setBlurRadius(blurRadius)
        glass.setChromaticAberration(chromaticAberration)
        glass.setDispersion(dispersionR, dispersionB)
        glass.setBrightness(brightness)
        glass.setHighlightWidth(highlightWidth)
        glass.setShadowProperties(shadowColor, shadowSoftness)
        glass.setTransmittance(transmittance)
        glass.setLightDirection(lightDirX, lightDirY)
        glass.setSpecular(specular, shininess)
        glass.setRimStrength(rimStrength)
        glass.setCausticIntensity(causticIntensity)
        glass.setShowNormals(showNormals)
    }
...
/**
 * Extension to conveniently apply a [GlassFilter] to a [GlassFrameLayout].
 */
fun GlassFrameLayout.applyFilter(filter: GlassFilter) = filter.applyTo(this)
