...
/**
 * **GlassSlider** - An iOS-style liquid-glass slider with a draggable frosted thumb.
 *
 ...
 * ### Usage Example
 * ```kotlin
 * val slider = GlassSlider(context).apply {
 *     setMaxValue(100f)
 *     setValue(42f)
 ...
 * ```
 *
 * XML attributes: `psl_maxValue`, `psl_trackColor` (see [R.styleable.GlassSlider]).
 *
...
class GlassSlider @JvmOverloads constructor(
    ...
    private fun applySquish() {
        val v = abs(normVel).coerceAtMost(0.2f)
        val sx = scaleXSpring.value + v * 0.75f
        val sy = (scaleYSpring.value - v * 0.25f).coerceAtLeast(0.75f)
        thumb.apply {
            pivotX = thumbW / 2f
            pivotY = thumbH / 2f
            scaleX = sx
    ...
    private val touchListener = OnTouchListener { _, e ->
        when (e.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                ...
                thumbCapturePending = false
                lastRawX = e.rawX
                scaleXSpring.animateTo(1.28f)
                scaleYSpring.animateTo(0.9f)
    ...
    init {
        ...
        context.obtainStyledAttributes(attrs, R.styleable.GlassSlider).use { a ->
            maxValue = a.getFloat(R.styleable.GlassSlider_psl_maxValue, 100f)
            val c = a.getColor(R.styleable.GlassSlider_psl_trackColor, accentColor)
            accentColor = c
            track.setFillColor(c)
            restBlur = a.getFloat(R.styleable.GlassSlider_psl_thumbBlurRadius, restBlur)
            pressChromatic = a.getFloat(R.styleable.GlassSlider_psl_thumbChromaticAberration, pressChromatic)
            thumbIOR = a.getFloat(R.styleable.GlassSlider_psl_thumbIOR, thumbIOR)
            thumbBrightness = a.getFloat(R.styleable.GlassSlider_psl_thumbBrightness, thumbBrightness)
            thumbNormalStrength = a.getFloat(R.styleable.GlassSlider_psl_thumbNormalStrength, thumbNormalStrength)
            thumbDisplacementScale = a.getFloat(R.styleable.GlassSlider_psl_thumbDisplacementScale, thumbDisplacementScale)
            thumbShadowColor = a.getColor(R.styleable.GlassSlider_psl_thumbShadowColor, thumbShadowColor)
            thumbShadowSoftness = a.getFloat(R.styleable.GlassSlider_psl_thumbShadowSoftness, thumbShadowSoftness)
            thumbThicknessPx = a.getDimension(R.styleable.GlassSlider_psl_thumbThickness, -1f)
            thumbHighlightWidth = a.getFloat(R.styleable.GlassSlider_psl_thumbHighlightWidth, -1f)
            thumbHeightBlurFactor = a.getFloat(R.styleable.GlassSlider_psl_thumbHeightBlurFactor, -1f)
            thumbMinSmoothing = a.getFloat(R.styleable.GlassSlider_psl_thumbMinSmoothing, thumbMinSmoothing)
            thumbCornerRadiusPx = a.getDimension(R.styleable.GlassSlider_psl_thumbCornerRadius, -1f)
            thumbRefractionInset = a.getFloat(R.styleable.GlassSlider_psl_thumbRefractionInset, -1f)
            thumbEdgeFalloff = a.getFloat(R.styleable.GlassSlider_psl_thumbEdgeRefractionFalloff, -1f)
            thumbParallaxScale = a.getFloat(R.styleable.GlassSlider_psl_thumbParallaxScale, thumbParallaxScale)
            thumbShowNormals = a.getBoolean(R.styleable.GlassSlider_psl_thumbShowNormals, false)
            thumbGlassColor = a.getColor(R.styleable.GlassSlider_psl_thumbColor, thumbGlassColor)
        }
    ...
    /** @return Whether the user is currently dragging the slider. */
    ...
    /**
     * Returns the underlying [GlassFrameLayout] used as the draggable thumb.
     * Useful for advanced customization beyond the `setThumb*` helpers.
     */
    fun getThumb(): GlassFrameLayout = thumb
