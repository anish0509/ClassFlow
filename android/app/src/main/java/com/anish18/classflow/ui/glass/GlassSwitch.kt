...
/**
 * **GlassSwitch** - An iOS-style liquid-glass toggle with tap and drag support.
 *
 ...
 * ### Usage Example
 * ```kotlin
 * val toggle = GlassSwitch(context).apply {
 *     setOn(true, animate = false)
 *     setOnToggleChangedListener { on -> Log.d("Switch", "on=$on") }
 * }
 * ```
 *
 * XML attributes: `psw_isOn`, `psw_trackHeight` (see [R.styleable.GlassSwitch]).
 *
...
class GlassSwitch @JvmOverloads constructor(
    ...
    private val thumb = GlassFrameLayout(context)
    ...
    private fun applyPressState(t: Float) {
        ...
        // Keep appearance stable and unaffected by hold/press
        thumb.setBlurRadius(restBlur)
        thumb.setChromaticAberration(0f)
        thumb.setLensRefractionScale(0.5f)
        thumb.setHeightBlurFactor(restHBF)
        overlay.alpha = 1f
    }

    private fun applySquish() {
        // Keep scale constant, no squish, stretch or dynamic deformation
        thumb.pivotX = thumbW / 2f
        thumb.pivotY = thumbH / 2f
        thumb.scaleX = 1f
        thumb.scaleY = 1f
        thumb.updateBackdropScale()
    }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        when (event.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                ...
                // Keep the resting glass shape on hold/press
                scaleXSpring.animateTo(1f)
                scaleYSpring.animateTo(1f)
                pressSpring.animateTo(0f)
                
                // Capture the background backdrop once on touch start
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (didDrag) {
                    ...
                    // Snap the position spring directly so release animations start smoothly
                    posSpring.snapTo(newFrac)
                }
                return true
            }
            ...
            MotionEvent.ACTION_UP -> {
                ...
                // Restore default scale and refraction values
                scaleXSpring.animateTo(1f)
                scaleYSpring.animateTo(1f)
                pressSpring.animateTo(0f)
                
                // Recapture the backdrop once at final rest
                postDelayed({
                    thumb.updateBackground()
                }, 100)
            ...
            MotionEvent.ACTION_CANCEL -> {
                ...
                normVel = 0f
                velTracker?.recycle(); velTracker = null
                // Cancel gesture: snap back to current state, DO NOT toggle
                posSpring.animateTo(if (isOn) 1f else 0f)
    ...
    init {
        ...
        context.theme.obtainStyledAttributes(attrs, R.styleable.GlassSwitch, 0, 0).apply {
            try {
                isOn = getBoolean(R.styleable.GlassSwitch_psw_isOn, false)
                trackH = getDimension(R.styleable.GlassSwitch_psw_trackHeight, dp(28f))
                onColor = getColor(R.styleable.GlassSwitch_psw_onColor, onColor)
                offColor = getColor(R.styleable.GlassSwitch_psw_offColor, offColor)
                restBlur = getFloat(R.styleable.GlassSwitch_psw_thumbBlurRadius, restBlur)
                pressChromatic = getFloat(R.styleable.GlassSwitch_psw_thumbChromaticAberration, pressChromatic)
                thumbIOR = getFloat(R.styleable.GlassSwitch_psw_thumbIOR, thumbIOR)
                thumbBrightness = getFloat(R.styleable.GlassSwitch_psw_thumbBrightness, thumbBrightness)
                thumbNormalStrength = getFloat(R.styleable.GlassSwitch_psw_thumbNormalStrength, thumbNormalStrength)
                thumbDisplacementScale = getFloat(R.styleable.GlassSwitch_psw_thumbDisplacementScale, thumbDisplacementScale)
                thumbShadowColor = getColor(R.styleable.GlassSwitch_psw_thumbShadowColor, thumbShadowColor)
                thumbShadowSoftness = getFloat(R.styleable.GlassSwitch_psw_thumbShadowSoftness, thumbShadowSoftness)
                thumbThicknessPx = getDimension(R.styleable.GlassSwitch_psw_thumbThickness, -1f)
                thumbHighlightWidth = getFloat(R.styleable.GlassSwitch_psw_thumbHighlightWidth, -1f)
                thumbHeightBlurFactor = getFloat(R.styleable.GlassSwitch_psw_thumbHeightBlurFactor, -1f)
                thumbMinSmoothing = getFloat(R.styleable.GlassSwitch_psw_thumbMinSmoothing, thumbMinSmoothing)
                thumbCornerRadiusPx = getDimension(R.styleable.GlassSwitch_psw_thumbCornerRadius, -1f)
                thumbRefractionInset = getFloat(R.styleable.GlassSwitch_psw_thumbRefractionInset, -1f)
                thumbEdgeFalloff = getFloat(R.styleable.GlassSwitch_psw_thumbEdgeRefractionFalloff, -1f)
                thumbShowNormals = getBoolean(R.styleable.GlassSwitch_psw_thumbShowNormals, false)
                thumbGlassColor = getColor(R.styleable.GlassSwitch_psw_thumbColor, thumbGlassColor)
            } finally {
                recycle()
            }
        }
    ...
    private fun setupThumb() {
        ...
        thumb.requestLayout()
        GlassLiquidGlass.applyBase(thumb)
